...
from ragify.agents import RAGAgent, PipelineAgent, get_default_tools, agent_registry
...
def main():
    ...
    # 1. 演示RAGAgent
    print("\n1. RAGAgent示例...")
    rag_agent = RAGAgent(name="demo_rag_agent")
    ...
    for query in rag_queries:
        ...
        print(f"Agent回答: {result['response']}")

    # 2. 演示带工具的RAGAgent
    print("\n\n2. 带工具的RAGAgent示例...")
    rag_agent_with_tools = RAGAgent(name="demo_rag_tools")
    rag_agent_with_tools.add_tools(get_default_tools())
    ...
    for query in tool_queries:
        print(f"\n用户问题: {query}")
        print("-" * 60)
        result = rag_agent_with_tools.invoke(query)
        print(f"Agent回答: {result['response']}")

    # 3. 演示PipelineAgent
    print("\n\n3. PipelineAgent示例...")
    pipeline_agent = PipelineAgent(name="demo_pipeline")
    ...
    for query in pipeline_queries:
        print(f"\n用户问题: {query}")
        print("-" * 60)
        result = pipeline_agent.invoke(query)
        print(f"Agent回答: {result['response']}")

    # 4. 演示可用的Agent类型
    print("\n\n4. 可用的Agent类型...")
    print(f"已注册的Agent类型: {', '.join(agent_registry.list_agents())}")
