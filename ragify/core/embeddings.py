...
class DashScopeEmbeddings(Embeddings):
    """DashScope嵌入模型，直接使用httpx调用避免tokenization问题"""
...
class EmbeddingGenerator:
    ...
    def _initialize_embeddings(self) -> Embeddings:
        ...
        初始化嵌入模型
        """
        if self.embedding_provider in ("openai", "dashscope"):
            # 初始化OpenAI/DashScope嵌入
            import os
            api_key_env = self.config.get("embeddings.api_key_env")
            api_key = os.environ.get(api_key_env) if api_key_env else None
            if not api_key:
                api_key = self.config.get("embeddings.api_key")
            if not api_key:
                api_key = os.environ.get("DASHSCOPE_API_KEY") or os.environ.get("OPENAI_API_KEY")
            base_url = self.config.get("embeddings.base_url")
            ...
            return OpenAIEmbeddings(
                model=self.embedding_model,
                model_kwargs={"dimensions": self.embedding_dimensions}
            )
